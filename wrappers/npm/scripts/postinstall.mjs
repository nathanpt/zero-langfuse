#!/usr/bin/env node
// postinstall: fetch the prebuilt `zero-langfuse` binary for this platform from
// the matching GitHub Release and place it next to bin/zero-langfuse.js, which
// execs it.
//
// Mirrors scripts/install.sh and scripts/install.ps1. The asset-name scheme is
// the source of truth in .goreleaser.yaml:
//   zero-langfuse_{version}_{linux|darwin|windows}_{amd64|arm64}.{tar.gz|zip}
// with a SINGLE combined `zero-langfuse_{version}_checksums.txt` (sha256sum
// format: `<hash>  <filename>` per line).
//
// Safety: HTTPS-only download from the pinned repo, SHA-256 verification against
// the release's combined checksums file, and extraction that never trusts
// archive paths — we extract to a temp dir and copy only the known binary
// basename into place, so a crafted archive cannot write outside the package
// (no zip-slip).
//
// Env overrides (testing / mirrors / locked-down installs):
//   ZLF_SKIP_DOWNLOAD=1      skip entirely, exit 0 (wrapper will guide if run)
//   ZLF_INSTALL_DRY_RUN=1    print the resolved plan as JSON, no network, exit 0
//   ZLF_INSTALL_PLATFORM=…   override process.platform (linux|darwin|win32)
//   ZLF_INSTALL_ARCH=…       override process.arch (x64|arm64)
//   ZLF_REPO=owner/name      override the GitHub repo (default nathanpt/zero-langfuse)
//   ZLF_GITHUB_BASE_URL=…    override the download host (default https://github.com)
//   ZLF_ALLOW_INSECURE_DOWNLOAD=1  allow http origins (local testing only)
...
const REPO = process.env.ZLF_REPO || 'nathanpt/zero-langfuse';
const BASE = (process.env.ZLF_GITHUB_BASE_URL || 'https://github.com').replace(/\/+$/, '');
// The checksums file is fetched from the same origin as the archive, so TLS
// authenticity is the only real integrity control — require https unless
// explicitly overridden for local testing (e.g. a localhost mirror).
const ALLOW_INSECURE = process.env.ZLF_ALLOW_INSECURE_DOWNLOAD === '1';
const MAX_DOWNLOAD_BYTES = 512 * 1024 * 1024;

function fail(message) {
  console.error(`[zero-langfuse] ${message}`);
  process.exit(1);
}

function warnSkip(message) {
  // Exit 0 so an unsupported platform or an opt-out does not break `npm install`;
  // bin/zero-langfuse.js reports a clear "no native binary" message if the user runs it.
  console.error(`[zero-langfuse] ${message}`);
  process.exit(0);
}

// Maps GoReleaser GOOS/GOARCH tokens. GoReleaser emits linux/darwin/windows and
// amd64/arm64 — NOT Zero's macos/x64.
...
if (process.env.ZLF_SKIP_DOWNLOAD === '1') {
  warnSkip('ZLF_SKIP_DOWNLOAD=1 set — skipping native binary download.');
}

const rawPlatform = process.env.ZLF_INSTALL_PLATFORM || process.platform;
const rawArch = process.env.ZLF_INSTALL_ARCH || process.arch;
...
// The release matrix covers linux/darwin/windows × amd64/arm64, so every
// resolved (platform, arch) pair has an asset — no special-case skip (unlike
// Zero, which ships no windows-arm64).

const ext = platform === 'windows' ? 'zip' : 'tar.gz';
const binaryName = platform === 'windows' ? 'zero-langfuse.exe' : 'zero-langfuse';
const tag = `v${VERSION}`;
const assetName = `zero-langfuse_${VERSION}_${platform}_${arch}.${ext}`;
const checksumName = `zero-langfuse_${VERSION}_checksums.txt`;
const assetUrl = `${BASE}/${REPO}/releases/download/${tag}/${assetName}`;
const checksumUrl = `${BASE}/${REPO}/releases/download/${tag}/${checksumName}`;

if (process.env.ZLF_INSTALL_DRY_RUN === '1') {
...
const markerPath = join(packageRoot, '.zero-langfuse-binary-version');
  ...
  if (!ALLOW_INSECURE && !url.startsWith('https://')) {
    fail(
      `refusing insecure download origin for ${url}: only https:// is allowed ` +
        `(set ZLF_ALLOW_INSECURE_DOWNLOAD=1 to override for local testing).`,
    );
  }
  let response;
  try {
...
// Parse a sha256sum-style file (the release's combined checksums.txt). Anchored
// per line, and when the line carries a filename field it must equal the
// requested asset, so a checksum file cannot misattribute a digest.
function parseSha256(text, wantName) {
  ...
  fail(`could not find a SHA-256 digest for ${wantName} in ${checksumName}`);
}
...
async function main() {
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'zero-langfuse-install-'));
  try {
    ...
    // Copy only the known binary basename into the package root. We never honor
    ...
    console.error(`[zero-langfuse] installed ${binaryName} ${VERSION} (${platform}-${arch}, ${sizeKb} KB).`);
  } finally {
